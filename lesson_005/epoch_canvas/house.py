...
def draw_house(_x=300, _y=10):
    ...
    for y in range(house_height // (brick_height+brick_seam)):
        ...
        if 0 < y <= house_height // (brick_height+brick_seam):
            line_point1 = sd.get_point(x=_x, y=_y+y*(brick_height+brick_seam))
            line_point2 = sd.get_point(x=_x+house_length-1, y=_y+y*(brick_height+brick_seam))
        ...
        for x in range(start_x, _x + house_length, brick_length + brick_seam):
            line_point1 = sd.get_point(x=x, y=_y+y*(brick_height+brick_seam))
            line_point2 = sd.get_point(x=x, y=_y+(y+1)*(brick_height+brick_seam))
            sd.line(start_point=line_point1, end_point=line_point2, color=seam_color, width=brick_seam)
    bottom_point = sd.get_point(x=_x+(brick_length+brick_seam)*3-2, y=_y+(brick_height+brick_seam)*6+2)
